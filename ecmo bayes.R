...
avec = seq(0, 20, length.out=100)
bvec = seq(0, 20, length.out=100)
...
contour(pEvec, pCvec, prior2Matrix * jacobean)
lik2Matrix = matrix(apply(abgrid, 1, lik2), 
	nrow=length(avec))
contour(pEvec, pCvec, lik2Matrix * jacobean,
	add=T)
points(ecmoProportions, col='red', pch='.', cex=10)
